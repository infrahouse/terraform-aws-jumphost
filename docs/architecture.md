...
- `ec2:DeleteTags` — restricted to the `InspectorEc2Exclusion` key, on instances carrying
  this module's `created_by_module` tag, so Puppet can drop the tag after patching
...
4. `profile::boot_security_upgrade` applies pending security updates, then removes the
   `InspectorEc2Exclusion` tag the ASG set at launch.
5. When Puppet finishes, `/var/run/puppet-done` is created.

Step 4 exists because AWS Inspector otherwise scans an instance before
`unattended-upgrades` has run: it opens a finding for whatever was pending and closes it
on the next upgrade, but by then it has reopened the vulnerability group, and a group old
enough to be reopened that way breaks the remediation SLA. Excluding the instance until it
is patched means Inspector's first scan sees a patched host.

The exclusion is fail-open: an instance that launches tagged and never has the tag removed
is invisible to Inspector permanently, and silently. See
[Jumphost Missing from Inspector Findings](troubleshooting.md#jumphost-missing-from-inspector-findings).
...
The ASG also propagates `InspectorEc2Exclusion` at launch, which Puppet deletes from the
instance once security updates are applied. `propagate_at_launch` tags are written only at
launch, so the ASG never re-applies it to a running instance and deleting it does not show
as Terraform drift; the tag/untag cycle repeats on every launch, which is the intent.
